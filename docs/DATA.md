...
| `nhl.scores`, `nfl.scores`, `ncaaf.scores`, `mlb.scores` | sources | list of game dicts for the slate (empty when beyond `show_games_within_days`; college trims it to the `slate` setting — ranked games, your conferences, or all — with favourites always in; NHL and MLB lead with last night's results until `sports.game_day_rollover_hour`, NFL and college carry the whole week's) |
