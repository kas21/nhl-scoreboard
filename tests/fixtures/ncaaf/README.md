...
| `espn_teams.json` | `…/football/college-football/teams?limit=1000` — the 138 FBS schools with colours and logo URLs, spelled the way that endpoint spells them (`AF`, `BUF`, `JXST` for our `AFA`, `BUFF`, `JVST`), plus a satellite campus sharing Ohio State's `OSU` code. The real endpoint ignores `groups` and returns ~760 programmes; the extra rows here stand in for that |
